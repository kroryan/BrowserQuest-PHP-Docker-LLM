define(['jquery', 'storage'], function($, Storage) {
    ...
    var App = Class.extend({
        ...
        translateUI: function() {
            ...
            // Cargar el módulo i18n
            require(['i18n'], function(i18n) {
                // Aplicar traducciones a todos los elementos DOM con atributos data-i18n
                i18n.translateDOM();
                
                // Traducciones específicas para elementos que no usan data-i18n
                $('#instructions .title').text(i18n.get('ui.instructions.title'));
                $('#respawn').text(i18n.get('ui.death.button'));
                $('#population #instance-population').find('span:first').text(i18n.get('ui.interface.players_in_instance'));
                $('#population #world-population').find('span:first').text(i18n.get('ui.interface.players_total'));
                
                // Traducciones de placeholders
                $('#nameinput').attr('placeholder', i18n.get('ui.create_character.name_placeholder'));
                
                log.info("UI translated to: " + i18n.currentLang);
            });
        }
    });
